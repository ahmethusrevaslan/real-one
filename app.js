...
function initApp() {
    ...
    // Check for verification token in URL (Simulated)
    const urlParams = new URLSearchParams(window.location.search);
    if (urlParams.has('verify_email')) {
        authService.verifyUser(urlParams.get('verify_email'));
        // Show success notification
        setTimeout(() => showNotification('E-posta adresi başarıyla doğrulandı!', 'success'), 500);
    }
...
function createProductCard(product, extraClass = '') {
    ...
    const badge = product.status !== 'Available'
        ? `<span class="product-badge sold" style="position:absolute; top:12px; left:12px; padding:6px 12px; font-size:10px; font-weight:600; text-transform:uppercase; background:#333; color:#888; z-index:2;">${product.status.toUpperCase()}</span>`
        : product.featured ? `<span class="product-badge" style="position:absolute; top:12px; left:12px; padding:6px 12px; font-size:10px; font-weight:600; text-transform:uppercase; background:#fff; color:#000; z-index:2;">FEATURED</span>` : '';
...
function updateCartUI() {
    ...
    if (state.cart.length === 0) {
        container.innerHTML = '<p style="text-align:center; opacity:0.5; margin-top:40px;">Your cart is empty.</p>';
    } else {
